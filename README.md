...
マス数・高さ上限の変更や「ことばで世界をつくる」は操作完了時にすぐ反映され、
表示される「元にもどす」から10秒間だけ直前の世界へ戻せます。

アプリ内の言語は **日本語 / English** を設定パネルで切り替えられます
(初回は macOS の言語で自動選択)。

世界と「村のきろく」は自動保存され、次に開いたとき続きから始まります。
セーブは直前の正常な状態もバックアップし、破損を検出した場合は自動で復元します。
ウィンドウの位置と大きさも次回起動時に引き継がれます。
